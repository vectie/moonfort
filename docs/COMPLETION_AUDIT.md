...
| Local macOS proof | `sandbox-exec` is behaviorally probed and the E2E has refusal coverage. A modern-macOS loader compatibility fix must pass the positive canonical-denial/scratch-write branch on a real host. | In progress |
| Local Linux proof | A bubblewrap/Landlock-grade backend must prove private mounts, denied network, limits, and descendant cleanup, or refuse with no fallback. | In progress |
