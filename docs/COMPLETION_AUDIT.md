...
| Explicit reviewed promotion | `cmd/promote-retention` requires the retention ID plus approval/command digests and explicit per-file mappings, then revalidates fingerprints and destination ancestry before atomic promotion. A real authenticated MoonDesk-shaped request through MoonClaw promoted only the selected file, consumed retention, refused replay/digest substitution, and returned no host paths. | Complete |
