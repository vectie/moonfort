...
| Complete command-output artifacts | Output that reaches the hard ceiling or exceeds the model budget is retained under an opaque random ID in a canonical private executor directory; small complete output stays inline without artifact I/O. A spilled receipt exposes only a configurable model projection plus a size/SHA-256/media-type/expiry/key-ID/HMAC reference. Reads bind the artifact to the run and approval/command digests and reject forged references, byte substitution, traversal, expiry, links, size drift, and digest mismatch. Expired signed pairs are swept. This proves executor-owned retention and verification; it does not replace live AEN output-limit or microVM evidence. | Complete at the executor contract; live AEN proof still required |
...
| MoonDesk control surface | Source diagnostics use an authenticated private-control-plane route and require an enforced, cleanup-verified receipt. The Phase-4 UI renders bounded backend/profile/limit/diff evidence, allows exactly one reviewed file per transaction, and calls the authenticated MoonClaw promotion-v4 broker without host paths. Durable, cleanup-unverified, and recovery-required outcomes lock retries; only a proven non-apply remains retryable. MoonFlow/updater operations refuse rather than execute on the host. | Complete for implemented sandbox operations; integrated checkout remains to be committed upstream |
